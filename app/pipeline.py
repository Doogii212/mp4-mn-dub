...
class GoogleTranslator(Translator):
    def __init__(self) -> None:
        try:
            ...
            raise RuntimeError(
                "deep-translator is required for translation. Install it in requirements.txt"
            ) from exc
        self._translator_class = _GoogleTranslator

    def translate(self, text: str, source_lang: Optional[str], target_lang: str) -> str:
        if not text.strip():
            return text
        source = source_lang or "auto"
        translator = self._translator_class(source=source, target=target_lang)
...
def translate_segments(
    ...
    for segment in segments:
        try:
            ...
            logger.exception("Translation failed for segment starting at %.2f", segment.start)
...
def run_pipeline(
    ...
    translator = translator or GoogleTranslator()
    job_id = uuid.uuid4().hex
    job_dir = output_dir / job_id
    job_dir.mkdir(parents=True, exist_ok=True)

    audio_path = job_dir / f"{job_id}_audio.wav"
    srt_path = job_dir / f"{job_id}_translated.srt"
    burned_path = job_dir / f"{job_id}_burned.mp4"
    soft_path = job_dir / f"{job_id}_soft.mp4"
